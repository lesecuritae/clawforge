...
Das Repository ist bereits eine Rust-/PostgreSQL-Plattform mit 18 Workspace-
Crates, 30 sqlx-Migrationen, React-Frontend und Docker-Compose-Deployment.
...
| --- | --- | --- | --- | --- |
| Beispiel-Secrets | `compose.yml`, `compose.bootstrap.yml`, `secret/src/lib.rs`, `scripts/validate-secrets.sh` | erreichbare frische Installation; bekannte Credentials übernehmen Bootstrap/interne APIs | fehlende, bekannte, gleiche und zu schwache Secrets verhindern den Start | umgesetzt; Regression in `scripts/test-secret-hardening.sh` |
| Notifier SSRF/Secret-Exfiltration | `notification/src/lib.rs`, `api/src/main.rs`, `notifier/src/main.rs` | Admin-/API-Missbrauch konfiguriert Secret-Referenz und Angreifer-URL | fremde Secret-ID, private/link-local IP, URL-Userinfo, Redirect und DNS-Rebinding werden abgewiesen | umgesetzt; gemeinsame Policy und Negativtests |
